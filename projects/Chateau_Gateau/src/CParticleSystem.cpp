...
namespace nou
{
	...
	void ParticleUtility::VerticalConeEmit(float tanTheta, float speed, glm::vec3& pos, glm::vec3& vel, float y)
	{
	...
	void CParticleSystem::Update(float deltaTime)
	{
		...
		{	
			...
			//Kill the particle if its lifetime has ended.
	...
	void CParticleSystem::setYSpeed(float x)
	{
		ySpeed = x;
	}
