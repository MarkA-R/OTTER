...
int Shader::__GetUniformLocation(const std::string& name) {
	...
	int result = -1;// If our entry was not found, we call glGetUniform and store it for next time
	if (it == _uniformLocs.end()) {   
		result = glGetUniformLocation(_handle, name.c_str());  
		_uniformLocs[name] = result;    }// Otherwise, we had a value in the map, return it
